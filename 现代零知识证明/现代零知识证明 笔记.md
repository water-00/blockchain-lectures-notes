...
|               | private      | public                                        |
| ------------- | ------------ | --------------------------------------------- |
| input         | signal input | 类似于数组b, 还不太清楚什么时候用public input |
...
# setup, get `proving_key.json` and `verification_key.json`
snarkjs setup -c <circuit JSON file name>

# calculating a witness (i.e. all the intermediate signals and the output) from `input.json`, will get `witness.json`
snarkjs calculatewitness

# creating the proof, using the `proving_key.json` and the `witness.json` files by default to generate `proof.json` and `public.json`
snarkjs proof

# verifying the proof, using `verification_key.json`, `proof.json` and `public.json` to verify that is valid.
snarkjs verify

# generating the solidity contract
